...
# 配置样本生成器 - 减少样本数量，加快训练过程
# N = 训练样本数量, M = 模拟步数
print("配置样本生成器...")
sample_generator = dynaplex.get_sample_generator(mdp, N=2000, M=500)  # 减少样本数量

save_filename = 'gc_lsn'
...
for gen in range(0, num_gens):
    ...
    with open(sample_path(gen), 'r') as json_file:
        ...
        model = ActorMLP(input_dim=mdp.num_flat_features(), 
                         hidden_dim=[256, 128, 128, 128],  # 从GC-LSN.json
        ...
        # 数据加载器
        dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=0)
...
for i, item in enumerate(comparison):
    if isinstance(item, dict):
        ...
        if 'mean' in item:
            if 'std' in item:
                print(f"{policy_names[i]:<10} {item['mean']:<15.2f} (std: {item['std']:.2f})")
            else:
...
try:
    ...
    final_save_path = dynaplex.filepath("", "GC-LSN")
    dynaplex.save_policy(final_policy, 
                         ...
                         final_save_path, 
                         device)
    print(f"最终策略保存为 GC-LSN.pth 和 GC-LSN.json")
except Exception as e:
    print(f"保存最终策略时出错: {e}")
